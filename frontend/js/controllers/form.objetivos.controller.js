class FormObjetivosController {
    constructor(mainController) {
        this.main = mainController;
    }

    async form_objetivos_guardar() {
        try {
            const objetivosTextos = this.form_objetivos_obtenerTextosDeTextareas('.objetivo-text');
            const metasTextos = this.form_objetivos_obtenerTextosDeTextareas('.meta-text');
            const granjasGalpones = this.form_objetivos_obtenerGranjasGalpones();

            if (!this.form_objetivos_validar(objetivosTextos, metasTextos, granjasGalpones)) return;

            const datosComunes = this.form_objetivos_obtenerDatosComunes();
            const id = document.getElementById('modalId').value;

            if (id) {
                await this.form_objetivos_actualizar(objetivosTextos, metasTextos, granjasGalpones, datosComunes);
            } else {
                await this.form_objetivos_crear(objetivosTextos, metasTextos, granjasGalpones, datosComunes);
            }

            this.main.modalObjetivos.modal_objetivos_cerrar();
    ...
    form_objetivos_obtenerTextosDeTextareas(selector) {
        return Array.from(document.querySelectorAll(selector))
            .map(textarea => textarea.value.trim())
            .filter(texto => texto);
    }

    form_objetivos_obtenerGranjasGalpones() {
    ...
    form_objetivos_validar(objetivos, metas, granjas) {
    ...
    form_objetivos_obtenerDatosComunes() {
        return {
    ...
    async form_objetivos_actualizar(objetivos, metas, granjasGalpones, datosComunes) {
        ...
        if (registrosParaCrear.length > 0) {
            ...
            for (const payload of registrosParaCrear) {
                ...
                if (primerIdExistente && idNuevo) {
                    await this.form_objetivos_copiarTareas(primerIdExistente, idNuevo);
                }
            }
        }
    ...
    async form_objetivos_crear(objetivos, metas, granjasGalpones, datosComunes) {
        const maxItems = Math.max(objetivos.length, metas.length);
        ...
        for (const combo of granjasGalpones) {
            for (let i = 0; i < maxItems; i++) {
                ...
                if (primerIdCreado === null && idNuevo) {
                    ...
                    await this.form_objetivos_copiarTareas(primerIdCreado, idNuevo);
                }
            }
        }
    ...
    async form_objetivos_copiarTareas(idOrigen, idDestino) {
        try {
    ...
    form_objetivos_editar(piloto) {
        ...
        this.main.modalObjetivos.modal_objetivos_cargarGranjasGalponesDeGrupo(grupoRegistros);
        this.main.modalObjetivos.modal_objetivos_cargarObjetivosMetasDeGrupo(grupoRegistros);
        ...
        this.main.modalObjetivos.modal_objetivos_toggle(true);
    }

    async form_objetivos_eliminar(id, piloto) {
