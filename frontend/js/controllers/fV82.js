class ControlObjetivosController {
    ...
    */
    ...
    async actualizarRegistrosExistentes(objetivos, metas, granjasGalpones, datosComunes) {
        ...
        console.log('📋 Registros encontrados del grupo:', grupoRegistros.length);
        
        // Eliminar todos los registros existentes del grupo
        for (const reg of grupoRegistros) {
            console.log('🗑️ Eliminando registro ID:', reg.id);
            await this.service.delete(reg.id);
        }
        
        console.log('✅ Registros eliminados, creando nuevos...');
        
        // Crear nuevos registros con las combinaciones actuales
        for (const combo of granjasGalpones) {
            for (const objetivo of objetivos) {
                for (const meta of metas) {
                    ...
                    console.log('➕ Creando registro:', combo.granja, objetivo, meta);
    ...
    async crearNuevosRegistros(objetivos, metas, granjasGalpones, datosComunes) {
        ...
        // Si hay al menos un dato en objetivos, metas, granjas o galpones, guardar una sola fila con los datos presentes y los demás vacíos
        const objetivo = objetivos[0] || '';
        const meta = metas[0] || '';
        let granja = '', galpon = '';
        if (granjasGalpones && granjasGalpones.length > 0) {
            granja = granjasGalpones[0].granja || '';
            galpon = granjasGalpones[0].galpon || '';
        }
        // Solo guardar si hay al menos un dato
        if (objetivo || meta || granja || galpon) {
            const payload = {
                ...datosComunes,
                granja,
                galpon,
    ...
    async guardarTarea() {
        try {
            ...
            if (idTarea) {
                // ✅ ACTUALIZAR SOLO EN EL OBJETIVO ACTUAL
                ...
                // ✅ CREAR EN TODOS LOS OBJETIVOS/METAS DEL PILOTO
                const registrosPiloto = this.datos.filter(d => d.piloto === this.objetivoActual.piloto);
                for (const reg of registrosPiloto) {
                    await this.service.createSubproceso({
                        ...datosBase,
                        id_objetivo: reg.id
                    });
                }
                window.SwalHelpers.showSuccess('Tarea creada correctamente en todos los objetivos/metas del piloto');
            }
