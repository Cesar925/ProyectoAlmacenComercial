...
require_once __DIR__ . '/../controllers/ObjetivoArchivosController.php';
...
$archivosController = new ObjetivoArchivosController($db);
...
// Rutas para archivos de objetivos
elseif ($request === 'POST' && strpos($path, '/objetivo-archivos/upload') !== false) {
    $archivosController->upload();
} elseif ($request === 'GET' && preg_match('#/objetivo-archivos/(.+)$#', $path, $matches)) {
    $archivosController->getByObjetivo($matches[1]);
} elseif ($request === 'DELETE' && preg_match('#/objetivo-archivos/delete/(.+)$#', $path, $matches)) {
    $archivosController->delete($matches[1]);
}
?>