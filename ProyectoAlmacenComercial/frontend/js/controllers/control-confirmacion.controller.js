class ControlConfirmacionController {
    ...
    datos.forEach(item => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-6 py-4 text-right">
                ${!esCerrado ? `
                    <button class="btn-validar bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1 rounded text-sm shadow flex items-center gap-2 mx-auto" data-accion="validar">
                        <i class="fas fa-check-double"></i> Validar
                    </button>
                ` : `
                    <div class="flex items-center gap-2 justify-center">
                        <span class="text-emerald-600 text-sm font-bold flex items-center gap-2">Cerrado</span>
                        <button class="btn-editar bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm shadow flex items-center gap-2" data-accion="editar">
        ...
        // Evento click al botón Validar
        const btn = tr.querySelector('.btn-validar');
        if (btn) {
            btn.addEventListener('click', () => this.abrirModal(item));
        }

        // Evento click al botón Editar
        const btnEditar = tr.querySelector('.btn-editar');
        if (btnEditar) {
            btnEditar.addEventListener('click', () => this.abrirModalEdicion(item));
        }

        tbody.appendChild(tr);
    });
    // IMPORTANTE: Aplicar permisos después de renderizar
    PermisosManager.ocultarBotonesSegunPermisos();
}
    ...
    abrirModalEdicion(item) {
        ...
        const lblObjetivo = document.getElementById('lblObjetivoModal');
        if (lblObjetivo) lblObjetivo.textContent = `Editar: ${item.objetivo || 'Sin título'}`;

        // Cargar datos ya guardados para edición
        const inputFechaInicioReal = document.getElementById('inputFechaInicioReal');
        const inputFechaReal = document.getElementById('inputFechaReal');
        const inputPresupuesto = document.getElementById('inputPresupuesto');
        const txtObs = document.getElementById('txtObservaciones');

        if (inputFechaInicioReal) inputFechaInicioReal.value = item.fecha_inicio_real || '';
        if (inputFechaReal) inputFechaReal.value = item.fecha_fin_real || '';
        if (inputPresupuesto) inputPresupuesto.value = item.presupuesto_ejecutado || '';
        if (txtObs) txtObs.value = item.observaciones_cierre || '';
    ...
    async guardarCierre() {
        ...
        const fechaInicioReal = document.getElementById('inputFechaInicioReal')?.value;
        ...
        if (!fechaInicioReal || !fechaReal || !presupuesto) {
