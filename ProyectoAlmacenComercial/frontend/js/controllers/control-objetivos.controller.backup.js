class ControlObjetivosController {
    constructor() {
        ...
        this.archivosService = new ObjetivoArchivosService();
        this.config = window.ControlObjetivosConfig;
        this.datos = [];
        this.mapaPilotoAIdProy = {}; // Mapa para encontrar id_proy por piloto
        this.idProyActual = null; // ID del proyecto actual para tareas
    ...
    setupTableEventListeners() {
        ...
        tbody.addEventListener('click', (e) => {
            ...
            const actions = {
                ...
                'descargar-archivo': () => this.descargarOVisualizarArchivo(id),
    ...
    async cargarDatos() {
        try {
            ...
            // Llenar mapa de piloto a id_proy
            this.mapaPilotoAIdProy = {};
            this.datos.forEach(item => {
                if (item.piloto && item.id_proy) {
                    this.mapaPilotoAIdProy[item.piloto] = item.id_proy;
                }
            });
            console.log('📍 Mapa de pilotos a id_proy:', this.mapaPilotoAIdProy);
        ...
        return `
            <tr class="grupo-row hover:bg-gray-50 transition-colors">
                ...
                ${isFirst ? `
                    ...
                    <td class="px-4 py-2 align-top" rowspan="${rowspan}"><span class="text-xs font-medium">${principal.contrato || '-'}</span></td>
        ...
        return `
            <td class="px-4 py-2 text-center">
                <div class="flex items-center justify-center gap-2">
                    ...
                    ` : '<span class="inline-block w-10 h-10"></span><span class="inline-block w-10 h-10"></span>'}
    ...
    mostrarModalNuevo() {
        ...
        // Limpiar campos de contrato y archivos
        const inputContrato = document.getElementById('objetivo-contrato');
        const inputArchivos = document.getElementById('objetivo-adjunto');
        const fileListDecorativo = document.getElementById('fileListDecorativo');
        
        if (inputContrato) inputContrato.value = '';
        if (inputArchivos) inputArchivos.value = '';
        if (fileListDecorativo) fileListDecorativo.innerHTML = '';

        const hoy = new Date().toISOString().split('T')[0]; // Formato YYYY-MM-DD
        document.getElementById('modalInicio').value = hoy;
        document.getElementById('modalFin').value = hoy;
    ...
    */
    ...
    editarRegistro(piloto) {
        ...
        // Cargar contrato
        const inputContrato = document.getElementById('objetivo-contrato');
        if (inputContrato) inputContrato.value = principal.contrato || '';
        
        // Cargar archivos adjuntos
        this.cargarArchivosAdjuntos(principal.id);
    ...
    async cargarArchivosAdjuntos(idObjetivo) {
        try {
            const resultado = await this.archivosService.getArchivos(idObjetivo);
            const fileListDecorativo = document.getElementById('fileListDecorativo');
            
            if (fileListDecorativo) {
                fileListDecorativo.innerHTML = '';
                
                if (resultado.archivos && resultado.archivos.length > 0) {
                    resultado.archivos.forEach(archivo => {
                        const elemento = document.createElement('div');
                        elemento.className = 'flex items-center gap-3 bg-green-50 border border-green-200 rounded-lg p-3 mt-2';
                        
                        const nombre = archivo.ruta_archivo.split('/').pop();
                        ...
                        elemento.innerHTML = `
                            ...
                            <div class="flex-1 truncate">
                                ...
                                <span class="text-xs text-gray-500 ml-2">[${ext}]</span>
                            </div>
                            <a href="../${archivo.ruta_archivo}" target="_blank" class="text-blue-500 hover:text-blue-700 text-sm font-bold px-2">
                                <i class="fas fa-download"></i>
                            </a>
                            <button type="button" class="btn-eliminar-archivo-existente text-red-500 hover:text-red-700 text-lg font-bold px-2" data-id-ruta="${archivo.id_ruta}">×</button>
                        `;
                        
                        elemento.querySelector('.btn-eliminar-archivo-existente').addEventListener('click', async (e) => {
                            e.preventDefault();
                            if (confirm('¿Eliminar archivo?')) {
                                await this.eliminarArchivoAdjunto(archivo.id_ruta);
                                await this.cargarArchivosAdjuntos(idObjetivo);
                            }
                        });
                        
                        fileListDecorativo.appendChild(elemento);
                    });
                }
            }
        } catch (error) {
    ...
    async eliminarArchivoAdjunto(idRuta) {
        try {
            await this.archivosService.deleteArchivo(idRuta);
            this.mostrarNotificacion('Archivo eliminado', 'success');
        } catch (error) {
    ...
    async descargarOVisualizarArchivo(idObjetivo) {
        try {
            const response = await this.archivosService.getArchivos(idObjetivo);
            const archivos = response.archivos || [];
            
            if (!archivos || archivos.length === 0) {
                this.mostrarNotificacion('No hay archivos adjuntos', 'info');
                return;
            }

            const archivo = archivos[0]; // Tomar el primer (y único) archivo
            const rutaArchivo = archivo.ruta_archivo;
            const nombreArchivo = archivo.ruta_archivo.split('/').pop();
            const esoPDF = nombreArchivo.toLowerCase().endsWith('.pdf');
            ...
            console.error('Error al descargar/visualizar archivo:', error);
            this.mostrarNotificacion('Error al acceder al archivo', 'error');
        }
    }
    ...
    async guardarRegistro() {
        try {
            ...
            let idObjetivoGuardado = null;

            // Verificar si es edición usando grupoIdsActual en lugar de modalId
            if (this.grupoIdsActual && this.grupoIdsActual.length > 0) {
                idObjetivoGuardado = this.grupoIdsActual[0];
                await this.actualizarRegistrosExistentes(objetivosTextos, metasTextos, granjasGalpones, datosComunes);
            } else {
                // Para nuevos registros, obtener el primer ID creado
                const resultado = await this.crearNuevosRegistros(objetivosTextos, metasTextos, granjasGalpones, datosComunes);
                idObjetivoGuardado = resultado?.idPrincipal;
            }

            // Guardar archivos si existen
            if (idObjetivoGuardado && this.archivosSeleccionados.length > 0) {
                const esEdicion = this.grupoIdsActual && this.grupoIdsActual.length > 0;
                const resultadoArchivos = await this.guardarArchivos(idObjetivoGuardado, esEdicion);
                if (!resultadoArchivos.success) {
                    this.mostrarNotificacion('Algunos archivos no se guardaron: ' + resultadoArchivos.error, 'warning');
                }
            }
    ...
    obtenerDatosComunesFormulario() {
        return {
            ...
            contrato: document.getElementById('objetivo-contrato')?.value || ''
        };
    }
    ...
    async crearNuevosRegistros(objetivos, metas, granjasGalpones, datosComunes) {
        ...
        //if (idNuevo) registrosCreados.push(idNuevo);
         if (idNuevo) {
                registrosCreados.push(idNuevo);
                
                // ✅ GUARDAR EL PRIMER ID COMO PRINCIPAL
                if (!idPrincipal) {
                    idPrincipal = idNuevo;
                }}
    ...
    async abrirSubprocesos(piloto, idObjetivo) {
        ...
        // Guardar id_proy para usarlo en tareas
        this.idProyActual = this.objetivoActual.id_proy;
        console.log('✅ id_proy guardado:', this.idProyActual);
    ...
    async cargarTareasPorPiloto(piloto) {
        try {
            ...
            // Obtener todos los id_proy de ese piloto (usamos el primero ya que comparten el mismo id_proy)
            if (registrosPiloto.length > 0 && registrosPiloto[0].id_proy) {
                const tareas = await this.service.getSubprocesos(registrosPiloto[0].id_proy);
    ...
    if (!this.idProyActual) {
        window.SwalHelpers.showError('Error: No hay proyecto seleccionado. Por favor abre las tareas primero.');
        return;
    }
    ...
    document.getElementById('tareaIdObjetivo').value = this.idProyActual;
    console.log('✅ Abriendo modal con id_proy:', this.idProyActual);
    ...
    async guardarTarea() {
        try {
            ...
            if (!this.idProyActual) {
                window.SwalHelpers.showError('Error: No se encontró el ID del proyecto');
                return;
            }

            console.log('🔍 Datos a guardar:', {
                idTarea: idTarea,
                idProyActual: this.idProyActual,
            ...
            const datosBase = {
                ...
                id_proy: this.idProyActual
            };
            
            console.log('📤 Enviando datos:', datosBase);
            ...
            if (idTarea) {
                ...
                // ✅ CREAR NUEVA TAREA
                await this.service.createSubproceso(datosBase);
                window.SwalHelpers.showSuccess('Tarea creada correctamente');
            }

            this.cerrarModalTarea();
            if (this.idProyActual) {
                const tareas = await this.service.getSubprocesos(this.idProyActual);
                this.tareasActuales = Array.isArray(tareas) ? tareas : [];
                this.renderizarTareas(this.tareasActuales);
            }
            await this.cargarDatos();
    ...
    async cargarGranjas() {
        try {
            ...
            this.poblarSelectFiltroGranjas();
    ...
    poblarSelectFiltroGranjas() {
    const selectFiltro = document.getElementById('filterGranja');
    
    if (!selectFiltro || !this.granjasDisponibles?.length) {
        console.warn('⚠️ No se puede poblar filtro de granjas');
        return;
    }
    
    console.log('🔄 Poblando select de filtros con', this.granjasDisponibles.length, 'granjas');
    
    selectFiltro.innerHTML = '<option value="">Todas</option>';
    
    this.granjasDisponibles.forEach(granja => {
        const option = document.createElement('option');
        // ✅ CAMBIAR: Guardar el texto completo como en la BD
        option.value = `${granja.codigo} - ${granja.nombre}`;
        option.textContent = `${granja.codigo} - ${granja.nombre}`;
        selectFiltro.appendChild(option);
    });
    
    console.log('✅ Select de filtros poblado correctamente');
}
...
setupArchivosListeners() {
    const inputArchivos = document.getElementById('objetivo-adjunto');
    if (inputArchivos) {
        inputArchivos.addEventListener('change', () => this.manejarSeleccionArchivos());
    }
}

manejarSeleccionArchivos() {
    const input = document.getElementById('objetivo-adjunto');
    const fileList = document.getElementById('fileListDecorativo');
    ...
    // ============ SOPORTAR MÚLTIPLES ARCHIVOS (descomenta para habilitar) ============
    /*
    ...
    this.archivosSeleccionados.forEach((file, index) => {
        ...
        const elemento = document.createElement('div');
        elemento.className = 'flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-lg p-3 mt-2';
        elemento.innerHTML = `
            <i class="fas fa-file text-blue-600"></i>
            <div class="flex-1 truncate">
                <span class="font-medium text-gray-700">${file.name}</span>
                <span class="text-xs text-gray-500 ml-2">${tamaño}</span>
            </div>
            <button type="button" class="btn-quitar-archivo text-red-500 hover:text-red-700 text-lg font-bold px-2" data-index="${index}">×</button>
        `;

        elemento.querySelector('.btn-quitar-archivo').addEventListener('click', (e) => {
            e.preventDefault();
            this.quitarArchivo(index);
        });

        fileList.appendChild(elemento);
    });
    */
    // ============ FIN SOPORTE MÚLTIPLES ARCHIVOS ============

    // ============ SOPORTAR UN SOLO ARCHIVO (comentar esta sección para múltiples) ============
    // Mostrar solo el primer archivo seleccionado
    if (input.files.length > 0) {
        ...
        elemento.innerHTML = `
            ...
            <button type="button" class="btn-quitar-archivo text-red-500 hover:text-red-700 text-lg font-bold px-2">×</button>
        `;
    ...
    // ============ FIN SOPORTE UN SOLO ARCHIVO ============
}

// ============ MÉTODO PARA MÚLTIPLES ARCHIVOS (descomenta si necesitas múltiples) ============
/*
quitarArchivo(index) {
    // Crear un nuevo FileList sin el archivo en el índice
    const dt = new DataTransfer();
    for (let i = 0; i < this.archivosSeleccionados.length; i++) {
        if (i !== index) {
            dt.items.add(this.archivosSeleccionados[i]);
        }
    }
    
    const input = document.getElementById('objetivo-adjunto');
    input.files = dt.files;
    this.manejarSeleccionArchivos();
}
*/
// ============ FIN MÉTODO PARA MÚLTIPLES ARCHIVOS ============

async guardarArchivos(idObjetivo, esEdicion = false) {
    const input = document.getElementById('objetivo-adjunto');
    ...
    try {
        // Si es edición, eliminar archivo anterior antes de subir el nuevo
        if (esEdicion) {
            try {
                const archivosExistentes = await this.archivosService.getArchivos(idObjetivo);
                if (archivosExistentes.archivos && archivosExistentes.archivos.length > 0) {
                    for (let archivo of archivosExistentes.archivos) {
                        await this.archivosService.deleteArchivo(archivo.id_ruta);
                    }
                }
            } catch (error) {
        ...
        const resultado = await this.archivosService.uploadArchivos(idObjetivo, input.files);
        // Limpiar después de subir
        input.value = '';
        this.archivosSeleccionados = [];
        document.getElementById('fileListDecorativo').innerHTML = '';
        return { success: true, archivos: resultado.archivos };
    } catch (error) {
